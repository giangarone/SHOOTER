export async function run({ page, check, id }) {
  ...
  check('Venom applies a ten-point poison tick for four seconds',
    result.base?.status === 'poison' && result.base.duration === 4 && result.base.power === 10,
    JSON.stringify(result));
  check('Venom does not scale with weapon damage',
