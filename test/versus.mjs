...
try {
  ...
  const results = await page.evaluate(async () => {
    ...
    // by captureRun/restoreRun - so `player.activeItem` is always the ACTIVE player's,
    ...
    const p2Pool = g.__poolForTest(g.player.activeItem);
    t('the box excludes the ACTIVE player\'s item',
      !p2Pool.includes('itemHeal') && p2Pool.length === Object.keys(g.__activeItemsForTest).length - 1,
      'pool=' + p2Pool.length);
    ...
    g.match.slots[0].player.activeItem = 'itemFreeze';
    ...
    t('a snapshot carries the slot',
      Object.prototype.hasOwnProperty.call(g.match.slots[0].player, 'activeItem')
      && Object.prototype.hasOwnProperty.call(g.match.slots[0].player, 'activeItemCharge'));
