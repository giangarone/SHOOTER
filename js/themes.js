...
// other theme's, role by role: every theme's rushers interchangeable with every
// other theme's, its brutes with every other theme's brutes. That is the same
// role-parity rule waves.js has always had, widened from six pools to a
// theme-by-role grid, and it is enforced in test/themes.mjs rather than by
// good intentions.
//
...
//
// Every one of the slots below names its FINAL type, including the ones that
// do not exist yet. `resolveRole` falls back to RUST's type for any name
...
// the schedule, the tests and the room theming all changing shape once per
// theme for as long as the roster is being built.
//
...
export const THEMES = {
  ...
  candy: {
    ...
    roles: { rusher: 'taffy', gunner: 'bonbon', brute: 'jawbreaker',
      artillery: 'poprock', support: 'sugarspinner', flier: 'cottonkite' },
  },
  swamp: {
  ...
  // The swarm, spent as currency. Every mechanic in the theme is about the
  // crowd BEING the resource: a tick's burst arms the ground the player is
  // winning, a nurse makes the crowd tougher with the same bullets that were
  // clearing it, a mite turns the crowd into shrapnel, and the boss makes the
  // crowd outright.
  hive: {
    name: 'HIVE',
    color: 0xffb300,
...
// independent draws: one pass through the deck plays each theme exactly
// once, so a run cannot see EMBER twice and never see RIME, and finishing
// the deck means the same thing in every run.
//
...
export function themeForWave(seed, n, force = null) {
  ...
  // wave 1, and with the themes being built one at a time that is most of
 ...
 * whole time the fights are being built. So a theme with no boss of its own
 * borrows from whichever bosses DO exist, chosen by its position in the
