...
export class SFX {
  ...
  donationStart() {
    this.noise({ t: 0.14, v: 0.12, f: 1600, mode: 'bandpass' });
    this.tone({ f: 180, f2: 360, t: 0.18, type: 'triangle', v: 0.15 });
  }
  donationTick(progress) {
    // Frame-driven, short voices: pausing or forfeiting leaves no scheduled
    // ticking behind, and wheel deceleration naturally spaces the clicks out.
    this.tone({ f: 650 + progress * 450, t: 0.035, type: 'triangle', v: 0.12 });
    this.noise({ t: 0.025, v: 0.045, f: 2600, mode: 'bandpass' });
  }
  donationLoss() {
    this.tone({ f: 260, f2: 180, t: 0.20, type: 'triangle', v: 0.14 });
  }
  donationComplete() {
    // The settled win gets its own ascent; collecting the floating reward
    // later still uses the ordinary item pickup sound.
