...
// What a conduited burn or poison ticks for: the statuses' own flat rates,
// exactly what VENOM and INCENDIARY pay (see BURN_TICK in
// enemies/shared.js), with no build multipliers read at all.
const CONDUIT_TICK_POWER = { burn: BURN_TICK, poison: POISON_TICK };
...
class Game {
  ...
  _lavaFloorTick(dt) {
    ...
    for (const e of this.enemies) {
      ...
      e.applyStatus('burn', 2, BURN_TICK);
    }
   ...
   *
   * The two damage-over-time effects pay the statuses' own flat tick, exactly
   * as VENOM and INCENDIARY do (see BURN_TICK in enemies/shared.js) - a
   * conduit's burn is worth what every burn in the game is worth.
   */
  _conduit() {
    const m = this.player.mods;
    ...
    for (const [mine, theirs] of CONDUIT_MAP) {
      ...
      for (const e of this.enemies) {
        ...
        e.applyStatus(theirs, Math.min(t, CONDUIT_TICK * 2), CONDUIT_TICK_POWER[theirs] || 0);
      }
    }
  ...
  _landShot(en, point, dir, dealt, burst, crit = false, head = false) {
    ...
    if (m.poisonTime) {
      en.applyStatus('poison', m.poisonTime * m.dotTime, POISON_TICK * m.poisonPower * m.dotPower);
    }
    if (m.burnTime) {
      en.applyStatus('burn', m.burnTime * m.dotTime, BURN_TICK * m.burnPower * m.dotPower);
    }
  ...
  _firePellet(muzzle, targets, spread, w, dmgMult = 1, crit = false) {
    ...
    if (m.stigmata > 0) {
      for (const en of this.enemies) {
        ...
        // figured through every multiplier the shot itself was, so the chip
        // stays ten percent of the shot the player actually fired.
  ...
  _playDeaths() {
    ...
    for (let i = 0; i < this._deathCount; i++) {
      ...
      if (m.burnSpread > 0 && this._deathBurn[i]) {
        ...
        if (best) {
          best.applyStatus('burn', m.burnTime, BURN_TICK * m.burnPower * m.dotPower);
  ...
  _addAsh(pos) {
    ...
    this._ash.push({
      ...
      power: BURN_TICK * m.ashPower * m.dotPower, radius: m.ashRadius, drip: 0,
  ...
  // patch is laid - the flat burn rate every fire in the game pays (see
  // BURN_TICK in enemies/shared.js), times the pick's own strength. One
  ...
  _hellfirePower() {
    const m = this.player.mods;
    return BURN_TICK * m.hellfirePower * m.dotPower;
  }
    ...
    if (laying) {
      ...
      if (began || dx * dx + dz * dz > FIRE_STEP * FIRE_STEP) {
        ...
        this._addFire(
          ...
          hot ? this._hellfirePower() : BURN_TICK * m.slideFire * m.dotPower,
  ...
  _updateBeatPicks() {
    ...
    // pay a shot's worth: a pick that paid one of the player's own shots per
    // beat was bought twice by a damage build, once per pull and once per
    // beat survived.
    //
   ...
   *
   * THE POWER IS THE STATUSES' OWN FLAT TICK, through BURN_TICK and POISON_TICK,
   * exactly as VENOM and INCENDIARY's are - so a poison the player is carrying
   * comes off a body at the rate every poison in the game ticks at, not at the
   * rate of whatever poisoned them. Scaled by MALADY like every other
   * damage-over-time in the game.
   *
  ...
  _splashback(en) {
    ...
    if (P.fire > 0) en.applyStatus('burn', P.fire * m.dotTime, BURN_TICK * m.dotPower);
    if (P.poison > 0) en.applyStatus('poison', P.poison * m.dotTime, POISON_TICK * m.dotPower);
  ...
  _updatePoisonSpread(dt) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      for (let j = 0; j < list.length; j++) {
        ...
        dst.applyStatus('poison', m.poisonTime * m.dotTime, POISON_TICK * m.poisonPower * m.dotPower);
        this.effects.burst(
  ...
  //
  // WHAT A SHARD IS WORTH IS WHAT A SHOT IS WORTH, four times over - the
  // player's own round through getEffectiveDamage, so every damage passive
  // item in the build feeds the ring exactly as it feeds the gun. Snapshotted
  // here, at the throw, rather than carried on the shard: a totem claimed
  // while eight shards are in the air must not rescale them.
  _reloadBurst() {
    const m = this.player.mods;
